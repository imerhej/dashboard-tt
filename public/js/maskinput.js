! function(factory) {
    ...
    $.mask = {
        ...
        caret: function(begin, end) {
            var range;
            if (0 !== this.length && !this.is(":hidden")) return "number" == typeof begin ?
                (end = "number" == typeof end ? end : begin, this.each(function() {
                ...
                })) : (this[0].setSelectionRange ? (begin = this[0].selectionStart, end = this[0].selectionEnd) : document.selection && document.selection.createRange && (range = document.selection.createRange(), begin = 0 - range.duplicate().moveStart("character", -1E5), end =
        ...
        mask: function(mask, settings) {
            ...
            return settings = $.extend({
                    ...
                    completed: null
                }, settings), defs = $.mask.definitions, tests = [], partialPosition = len = mask.length, firstNonMaskPos =
                null, $.each(mask.split(""), function(i, c) {
                    ...
                    function shiftR(pos) {
                        ...
                        for (i = pos, c = getPlaceholder(pos); len > i; i++)
                            if (tests[i]) {
                                if (j =
                                    seekNext(i), t = buffer[i], buffer[i] = c, !(len > j && tests[j].test(t))) break;
                                c = t
                            }
                    }
                    ...
                    function checkVal(allow) {
                        ...
                        for (i = 0, pos = 0; len > i; i++)
                            ...
                            } else buffer[i] === test.charAt(pos) && pos++, partialPosition > i && (lastMatch =
                                i);
                    ...
                    }).on("blur.mask", blurEvent).on("keydown.mask", keydownEvent).on("keypress.mask",
